...
import { ArrowRight, Sparkles, Zap, Globe, Upload, Shirt, Wand2, Clock, ShieldCheck, GitCompare } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "AI Fit Studio — Virtual Try-On" },
      {
        property: "og:description",
        content: "Try outfits virtually before you buy with AI-powered fashion previews.",
      },
    ],
  }),
  component: Home,
    ...
    <>
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 sm:px-8 md:py-24 lg:grid-cols-2 lg:items-center lg:gap-16">
          <div className="space-y-7">
            ...
            <h1 className="font-display text-5xl leading-[1.05] text-charcoal sm:text-6xl lg:text-7xl">
              Try Outfits <em className="not-italic text-gold">Virtually</em><br />
              Before You Buy
            </h1>
            <p className="max-w-lg text-base text-muted-foreground sm:text-lg">
              Upload your photo, choose an outfit, and see how it looks on you in seconds. No app, no waiting — just elegant fashion at your fingertips.
            </p>
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] border border-border bg-cream shadow-luxe">
              ...
              <div className="pointer-events-none absolute inset-x-6 bottom-6 flex items-center justify-between rounded-2xl bg-background/85 p-4 backdrop-blur-xl">
                <div>
                  <p className="text-[11px] uppercase tracking-widest text-muted-foreground">AI Preview</p>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <div className="mb-12 text-center">
          ...
          <h2 className="mt-3 font-display text-4xl text-charcoal sm:text-5xl">Three simple steps</h2>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {[
            { icon: Upload, title: "Upload Your Photo", desc: "Add a clear, front-facing full-body photo in good lighting." },
            { icon: Shirt, title: "Choose an Outfit", desc: "Browse our curated collection and pick a piece you'd love to try." },
            { icon: Wand2, title: "See Your AI Try-On", desc: "Our AI fits the outfit on you while preserving your pose and background." },
          ].map((step, i) => (
            <div key={step.title} className="relative rounded-3xl border border-border bg-card p-8 shadow-soft">
              <span className="absolute right-6 top-6 font-display text-5xl text-cream-deep">0{i + 1}</span>
        ...
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
          <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 font-display text-4xl text-charcoal sm:text-5xl">Featured outfits</h2>
            </div>
            <Link
              to="/collection"
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <div className="mb-12 text-center">
          ...
          <h2 className="mt-3 font-display text-4xl text-charcoal sm:text-5xl">Shop with total confidence</h2>
        </div>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { icon: Clock, title: "Save time", desc: "Skip the fitting-room queue and try styles instantly from home." },
            { icon: ShieldCheck, title: "Buy with confidence", desc: "See how a piece truly fits before committing to a purchase." },
            { icon: Globe, title: "Works in browser", desc: "No apps, no downloads — just open and try on outfits in seconds." },
            { icon: GitCompare, title: "Compare easily", desc: "Swap outfits quickly to compare looks side by side." },
          ].map((c) => (
            <div key={c.title} className="rounded-3xl border border-border bg-card p-7 shadow-soft transition hover:shadow-luxe">
